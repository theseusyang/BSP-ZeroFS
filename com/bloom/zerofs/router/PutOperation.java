...
import com.bloom.zerofs.messageformat.MetadataContentSerDe;
  ...
  class PutChunk {
    ...
    protected PartitionId getPartitionForPut(List<PartitionId> partitionIdsToExclude)
      ...
      if (partitions.isEmpty()) {
        throw new RouterException("No writable partitions available.", RouterErrorCode.AmberUnavailable);
      }
    ...
    private void processServerError(ServerErrorCode error) {
      // for puts, all errors are effectively Amber_Unavailable. One could argue that certain errors
      ...
      // result in Amber_Unavailable or UnexpectedInternalError.
      ...
      setChunkException(new RouterException("Could not complete operation, server returned: " + error,
          RouterErrorCode.AmberUnavailable));
    }
