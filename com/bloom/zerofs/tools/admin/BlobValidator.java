...
public class BlobValidator {
  ...
  public static void main(String args[]) {
    ...
    try {
      ...
      ArgumentAcceptingOptionSpec<String> AmberBlobIdListOpt =
      ...
      listOpt.add(AmberBlobIdListOpt);
      ...
      String blobIdListStr = options.valueOf(AmberBlobIdListOpt);
  ...
  private ServerErrorCode validateBlobOnReplica(BlobId blobId, ClusterMap clusterMap, ReplicaId replicaId,
    ...
    try {
      ...
      if (getResponse.getError() != ServerErrorCode.No_Error || serverResponseCode != ServerErrorCode.No_Error) {
        ...
        byte[] blobFromAmber = new byte[(int) blobData.getSize()];
        int blobSizeToRead = (int) blobData.getSize();
        int blobSizeRead = 0;
        while (blobSizeRead < blobSizeToRead) {
          blobSizeRead += blobData.getStream().read(blobFromAmber, blobSizeRead, blobSizeToRead - blobSizeRead);
        }
